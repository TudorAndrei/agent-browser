...
`codegen` captures successful actions as Chrome DevTools Recorder JSON or an `@playwright/test` spec. It is distinct from [Video Recording](/recording), which writes a WebM file.
...
`codegen status` reports active, restored, degraded, recovery-error, and cleanup-pending states. Use `codegen discard` to remove an unfinished or damaged journal.
