...
See [Debugging](/debugging) for console, error, dialog, trace, highlight, and DevTools workflows. See [Video Recording](/recording) for `record` workflows, [Codegen](/codegen) for reusable test artifacts, and [Profiler](/profiler) for performance profiles.
