...
Recording requires `ffmpeg` on `PATH`. See [Debugging](/debugging) for console, error, dialog, trace, highlight, and DevTools workflows. See [Video Recording](/recording) for recording requirements and `record` workflows, [Codegen](/codegen) for reusable test artifacts, and [Profiler](/profiler) for performance profiles.

## Codegen

`codegen` captures supported successful actions as Chrome DevTools Recorder JSON or an `@playwright/test` spec. It omits unsafe targets and unsupported actions and reports warnings. It is distinct from [Video Recording](/recording), which writes a WebM file.
...
`codegen status` reports active, restored, degraded, recovery-error, and cleanup-pending states. It also reports capture and security warning counts and projected Recorder and Playwright counts. `codegen stop` reports selected-format step counts and grouped warning codes. Use `codegen discard` to remove an unfinished or damaged journal.
