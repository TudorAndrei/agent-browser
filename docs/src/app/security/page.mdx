...
The stop response always gives a typed-value warning when the flow contains fill, set value, type, select, or upload. It gives a stronger warning when the target is a confirmed password input. These warnings do not include the captured value.
