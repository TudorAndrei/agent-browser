...
`codegen` captures typed values verbatim so generated Recorder flows and Playwright specs can replay the original interaction. This includes credentials entered into password fields. Review generated files before sharing or committing them. The in-progress sidecar and capture metadata are stored with owner-only permissions in the daemon socket directory, but they should still be treated as sensitive.
